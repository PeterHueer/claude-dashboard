...
loaders.agents = async function loadAgents() {
  ...
  _agentsData = await api('/api/agents');
  ...
  container.innerHTML = '<h2 class="text-lg font-bold mb-4 text-primary">Agents</h2>';
  initSearchBar(container, 'agents-search', 'Search agents…', renderAgentsList);
  renderAgentsList();
};

function renderAgentsList() {
  const q = document.getElementById('agents-search')?.value || '';
  const agents = filterByQuery(_agentsData, q, ['name', 'description', 'plugin']);
  const results = document.getElementById('agents-search-results');
  if (!results) return;

  if (!agents.length) {
    results.innerHTML = '<p class="text-sm opacity-50">No agents match your search.</p>';
    return;
  }
  ...
  results.innerHTML = Object.entries(grouped).map(([plugin, items]) => `
    <div class="mb-8">
